...
from ..losses import ICLoss, UPLoss, UALoss, CCLoss,Dist
...
def unknown_aware_nms(boxes, scores, labels, ukn_class_id=80, iou_thr=1.0):
...
@ROI_BOX_OUTPUT_LAYERS_REGISTRY.register()
class PUDetFastRCNNOutputLayers(CosineFastRCNNOutputLayers):
    """
    ...
    def __init__(
        ...
        self.ccl_iou_thr = ccl_iou_thr
        self.ccl_out_dim = ccl_out_dim
        self.ccl_center_init = ccl_center_init
        self.ccl_distance_type = ccl_distance_type

        self.ccL_loss = CCLoss( num_known_classes=self.num_known_classes,
                           feat_dim=self.ccl_out_dim,
                           init=self.ccl_center_init)
        self.representatives = self.ccL_loss.representatives
        self.encoder = MLP(self.cls_score.in_features, self.ccl_out_dim)
        # ual
        self.ual_start_iter = ual_start_iter
        self.ual_edl_start_iter = ual_edl_start_iter
        self.ual_loss_type = ual_loss_type
        self.ual_topk = ual_topk
        self.ual_weight = ual_weight
        self.ual_iou_weight = ual_iou_weight
        self.ual_lam = ual_lam

        self.ual_loss = UALoss(num_classes=self.num_classes,
                               num_known_classes=self.num_known_classes,
                               wi_start_iter=self.ual_edl_start_iter,
                               loss_type=self.ual_loss_type,
                               topk=self.ual_topk,lam=ual_lam)
    ...
    def from_config(cls, cfg, input_shape):
        ...
        ret.update({
            ...
            # ccl
            "ccl_weight": cfg.CCLOSS.WEIGHT,
            "ccl_out_dim": cfg.CCLOSS.OUT_DIM,
            "ccl_iou_thr": cfg.CCLOSS.IOU_THR,
            "ccl_center_init":cfg.CCLOSS.INIT,
            "ccl_distance_type": cfg.CCLOSS.TYPE,

            # fmt: on
            "ual_start_iter": cfg.UALOSS.START_ITER,
            "ual_edl_start_iter": cfg.UALOSS.EDL_START_ITER,
            "ual_loss_type": cfg.UALOSS.LOSS_TYPE,
            "ual_topk": cfg.UALOSS.TOPK,
            "ual_weight": cfg.UALOSS.WEIGHT,
            "ual_iou_weight": cfg.UALOSS.EDL_WEIGHT,
            "ual_lam": cfg.UALOSS.LAM,
        })
        return ret
    ...
    def get_ccl_loss(self, feat, gt_classes):
        pos_inds = (gt_classes != self.num_classes) # & (ious > self.ccl_iou_thr)
        feat = feat[pos_inds]
        label = gt_classes[pos_inds]
        ccl_loss = self.ccL_loss(feat, label, self.representatives, distance_type='hy')
        storage = get_event_storage()
        decay_weight = 1.0 - storage.iter / self.max_iters
        return {"ccl_loss":  self.ccl_weight * decay_weight * ccl_loss}

    def get_ual_loss(self, scores, gt_classes, ious):
        edl_loss, ious_loss, ual_loss = self.ual_loss(scores, gt_classes, ious)
        storage = get_event_storage()
        if storage.iter > self.ual_start_iter:
            ual_loss = ual_loss
        else:
            ual_loss = scores.new_tensor(0.0)
        decay_weight = 1.0 - storage.iter / self.max_iters
        return {
            "edl_iou_loss":  self.ual_weight*edl_loss + self.ual_iou_weight*self.ual_weight*decay_weight*ious_loss,
            "ual_loss": self.ual_weight *ual_loss
        }
    ...
    def losses(self, predictions, proposals):
        ...
        losses.update(self.get_ccl_loss(mlp_feat, gt_classes))
        ious = cat([p.iou for p in proposals], dim=0)
        # losses.update(self.get_ual_loss(scores, gt_classes, ious))
        losses.update(self.get_ual_loss(scores, gt_classes, ious))
